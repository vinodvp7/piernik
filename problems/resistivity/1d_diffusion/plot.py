...
eta = 0.05
a= 40
y_analytical = lambda t, x: 0.1/np.sqrt(1.0 + 4.0*a*eta*t) * np.exp(-(a*x**2)/(1.0 + 4.0*a*eta*t))
ts, L1s, L2s = [], [], []

data_to_plot = 'mag_field_y'

file = '/home/vinodvp/simdir/piernik/runs/1d_diffusion/res_tst_0000.h5'
...
plt.plot(x,y,label='t=0.0',linewidth=2.0,color='k')
plt.plot(x,y_analytical(0.0,x),label='t=0.0 (Analytical)',linewidth=2.0,color='r', linestyle='dashed')
L1 = np.mean(abs(y-y_analytical(0.0,x)))
L2 =np.sqrt( np.mean((y-y_analytical(0.0,x))**2))
ts.append(0.0); L1s.append(L1); L2s.append(L2)

file = '/home/vinodvp/simdir/piernik/runs/1d_diffusion/res_tst_0003.h5'
data, cell_dims, origin, spacing = load_and_stitch_data(file)
y=data[data_to_plot][0,0,:]
...
plt.xlim(-2.0,2.0)
plt.ylim(0,0.1)
plt.xticks([-2,-0.1,0,1,2])
plt.tight_layout()
plt.legend()
hdr = f"{'t':>4} {'L1':>12} {'L2':>12}"
rows = [f"{t:>4.1f} {l1:>12.3e} {l2:>12.3e}" for t,l1,l2 in zip(ts,L1s,L2s)]
plt.text(0.01, 0.98, "\n".join([hdr]+rows),
         transform=plt.gca().transAxes, va='top', ha='left', family='monospace',
