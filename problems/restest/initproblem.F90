...
!-----------------------------------------------------------------------------
   subroutine problem_initial_conditions
      ...
      real :: twopi, kx, ky, kk, A0

      ! set wave numbers and amplitude so that max(|Bx|)=max(|By|)=b0
      twopi = 6.283185307
      kx    = twopi          ! m = 1
      ky    = twopi          ! n = 1
      kk     = kx             ! kx = ky here
      A0    = b0 / kk         ! gives peak component amplitude = b0
                  ...
                  do k = cg%lhn(zdim,LO), cg%lhn(zdim,HI)
                     ...
                     if (fl%has_energy) then
                        ...
                        if (fl%is_magnetized) then
                           ...
                           cg%b(xdim,i,j,k) =  A0 * ky * sin(kx*xi) * cos(ky*yj)   ! = A0*ky*cos(ky*y)*sin(kx*x)
                           cg%b(ydim,i,j,k) = -A0 * kx * cos(kx*xi) * sin(ky*yj)  ! = -A0*kx*cos(kx*x)*sin(ky*y)
                           cg%b(zdim,i,j,k) =  0.0
                           cg%u(fl%ien,i,j,k) = cg%u(fl%ien,i,j,k) + &
