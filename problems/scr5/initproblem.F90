...
!      use global,      only: smallei
      ...
      fl => flind%all_fluids(flind%fluids)%fl
      cgl => leaves%first
      do while (associated(cgl))
         cg => cgl%cg
         do j = cg%lhn(ydim,LO), cg%lhn(ydim,HI)
            ...
            do i = cg%lhn(xdim,LO), cg%lhn(xdim,HI)
               ...
               do k = cg%lhn(zdim,LO), cg%lhn(zdim,HI)
                  zk = cg%z(k)
                  cg%u(fl%end + 1, i,j,k) = exp(-a*xi*xi -b*yj*yj)
                  cg%u(fl%end + 2,i,j,k) = 0.0
                  cg%u(fl%end + 3,i,j,k) = 0.0
                  cg%u(fl%end + 4,i,j,k) = 0.0
               enddo
            enddo
         enddo
         cgl => cgl%nxt
