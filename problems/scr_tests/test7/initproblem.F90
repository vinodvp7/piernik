...
! ------------------------------------------!
   ...
   real                  :: vl ,vr, x0
   real, dimension(nscr) :: ec0

   namelist /PROBLEM_CONTROL/  vl ,vr, x0, ec0 
         ...
         do while (associated(cgl))
            ...
            do j = cg%lhn(ydim,LO), cg%lhn(ydim,HI)
               ...
               do i = cg%lhn(xdim,LO), cg%lhn(xdim,HI)
                  ...
                  do k = cg%lhn(zdim,LO), cg%lhn(zdim,HI)
                     zk = cg%z(k)
                     cg%scr(scr_fluid%iescr, i,j,k) = ec0(p)  
                     cg%scr(scr_fluid%ixfscr,i,j,k) = 4./3. * cg%u(fl%imx,i,j,k)/cg%u(fl%idn,i,j,k) * cg%scr(scr_fluid%iescr,i,j,k)
