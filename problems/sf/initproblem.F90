...
module initproblem
   ...
   namelist /PROBLEM_CONTROL/  d0, p0, T0, ix, iy, iz, amp, mode
...
!-----------------------------------------------------------------------------
   ...
   subroutine problem_pointers
      ...
      use dataio_user,  only: user_tsl, user_attrs_pre, user_post_write_data
      use iosupport,    only: galdisk_tsl, galdisk_attrs_pre, galdisk_post_write_data
      use user_hooks,   only: problem_customize_solution, user_vars_arr_in_restart
#ifdef HDF5
      use dataio_user,  only: user_vars_hdf5, user_attrs_wr, user_attrs_rd
      use iosupport,    only: galdisk_vars_hdf5, read_initial_fld_from_restart, write_global_to_restart
#endif /* HDF5 */
      ...
      user_vars_arr_in_restart   => sn_init_supplement
      user_post_write_data       => galdisk_post_write_data
      problem_customize_solution => my_problem_customize_solution
#ifdef HDF5
...
!-----------------------------------------------------------------------------
 ...
 !-----------------------------------------------------------------------------
   ...
   !-----------------------------------------------------------------------------
      ...
      subroutine my_problem_customize_solution(forward)

#ifdef SNE_DISTR
#ifdef SN_DISTRIBUTION
            use sndistr,        only: sum_dmass_stars, sfr_dump, sfr2hdf, sne_dump
#ifdef NBODY
            use star_formation, only: SF, dmass_stars
#endif /* NBODY */
...
logical, intent(in) :: forward
...
#ifdef SN_DISTRIBUTION
#ifdef NBODY
            call SF(forward, sfr_dump, sfr2hdf, sne_dump)
            if (forward) sum_dmass_stars = sum_dmass_stars + dmass_stars
      ...
      end subroutine my_problem_customize_solution
...
!-----------------------------------------------------------------------------
   ...
   subroutine problem_initial_conditions
      ...
      use mpisetup,    only: proc  ! jrandpert
#ifdef NBODY
