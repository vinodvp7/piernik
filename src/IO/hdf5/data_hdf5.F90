...
#endif /* !ISO */
#ifdef RESISTIVE
      use resistivity,        only: eta_jn, eta_n
...
#ifdef RESISTIVE
         case ('curx')
            tab(:,:,:) = cg%w(wna%ind(eta_jn))%arr(xdim, RNG) / cg%q(qna%ind(eta_n))%arr(RNG)
         case ('cury')
            tab(:,:,:) = cg%w(wna%ind(eta_jn))%arr(ydim, RNG) / cg%q(qna%ind(eta_n))%arr(RNG)
         case ('curz')
            tab(:,:,:) = cg%w(wna%ind(eta_jn))%arr(zdim, RNG) / cg%q(qna%ind(eta_n))%arr(RNG)
#endif /* RESISTIVE */
#ifdef TRACER
         case ("trcr")
