...
module global
   ...
   public :: cleanup_global, init_global, &
        ...
        &    divB_0_method, cc_mag, glm_alpha, use_eglm, cfl_glm, ch_grid, w_epsilon, psi_bnd, ord_mag_prolong, ord_fluid_prolong, which_solver, which_solver_type
   ...
   integer         :: which_solver             !< one of RTVD_SPLIT, HLLC_SPLIT or RIEMANN
   integer         :: which_solver_type        !< For Riemann solver it can be either split or unsplit
   ...
   character(len=cbuff_len)      :: solver_type       !< allow to switch between dimensionally split and unsplit riemann solver
...
!<
   subroutine init_global
      ...
      use constants,  only: big_float, one, PIERNIK_INIT_DOMAIN, INVALID, DIVB_CT, DIVB_HDC, &
           ...
           &                RTVD_SPLIT, HLLC_SPLIT, RIEMANN, GEO_XYZ, V_INFO, V_DEBUG, V_ESSENTIAL, SPLIT, UNSPLIT
      ...
      if (master) then
         select case (which_solver_type)
            ...
            case (UNSPLIT)
               call printinfo("    (M)HD solver type: UNSPLIT RIEMANN.", V_INFO)
            case default
               call die("[global:init_global] unrecognized solver_type")
         end select
      endif
#ifdef MAGNETIC
      if (master) then
