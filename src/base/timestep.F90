...
!<
   subroutine time_step(dt, flind, main_call)
      ...
      use constants,          only: one, two, zero, half, pMIN, pMAX, UNSPLIT
      use dataio,             only: write_crashed
      use dataio_pub,         only: tend, msg, warn
      use fargo,              only: timestep_fargo
      use fluidtypes,         only: var_numbers
      use global,             only: t, dt_old, dt_full, dt_max_grow, dt_initial, dt_min, dt_max, nstep, repetitive_steps, which_solver_type
...
! Timestep computation
      ...
      if (which_solver_type == UNSPLIT) then
               dt = min(min(dt, dt_max), ((tend-t)) + (two*epsilon(one)*((tend-t))))    !< If unsplit we are moving 1 x dt so we dont need the half factor
      else
