...
!>
   ...
   subroutine update_interaction_term(cg, istep, at_source)
      ...
      if (.not. disable_streaming) then
         do ns = 1, flind%nscr 
            ...
            &  (flind%scr(ns)%gam * Bxyz * cg%w(uhi)%arr(iarr_all_escr(ns),:,:,:))))/(cg%w(sgmd)%arr(2*(ns - 1) + xdim ,:,:,:) + &
            & ( abs(bdotpc) * sqrt(cg%w(uhi)%arr(iarr_all_dn(1) ,:,:,:)) * vmax / &
   ...
   subroutine update_vdiff(cg, istep)
      ...
      use grid_cont,          only: grid_container
      use named_array_list,   only: wna
      use constants,          only: xdim, ydim, zdim, first_stage, sgmn, v_diff, uh_n, LO, HI
      use global,             only: integration_order
      ...
      associate( sd => cg%w(wna%ind(sgmn))%arr, v  => cg%w(wna%ind(v_diff))%arr)
