...
contains
   ...
   subroutine apply_scr_source(cg,istep)
      ...
      use initstreamingcr,  only: vmax, disable_streaming, disable_feedback, use_escr_floor, escr_floor, dt_scr, nsubcount, Nsub, omega
      ...
      real                                       :: gpcx, gpcy, gpcz, ec_source_, new_eg, dE, dFx, dFy, dFz, vi1, vi2, vi3
#ifdef MAGNETIC
      ...
      & i = cg%lhn(xdim,LO):cg%lhn(xdim,HI))
         dE = 0.0 ; dFx = 0.0; dFy = 0.0; dFz = 0.0           ! Zero in the feedback
         do ns = 1, scrind%nscr
#ifdef MAGNETIC
            ...
            new_ec = ec - m12 * f1 / m22 - m13 * f2 / m33 - m14 * f3 / m44
            ...
            vi1 = (cg%w(fldi)%arr(iarr_all_mx(1), i, j, k) - (newf1 - f1)/vmax)/ cg%w(fldi)%arr(iarr_all_dn(1), i, j, k)
            vi2 = (cg%w(fldi)%arr(iarr_all_my(1), i, j, k) - (newf2 - f2)/vmax)/ cg%w(fldi)%arr(iarr_all_dn(1), i, j, k)
            vi3 = (cg%w(fldi)%arr(iarr_all_mz(1), i, j, k) - (newf3 - f3)/vmax)/ cg%w(fldi)%arr(iarr_all_dn(1), i, j, k)
            ...
            st = cg%w(rtmi)%arr(stheta,i,j,k)
#endif /* MAGNETIC */
            v1 = (1.0 - omega) * cg%w(fldi)%arr(iarr_all_mx(1), i, j, k)/cg%w(fldi)%arr(iarr_all_dn(1), i, j, k) + omega * vi1
            v2 = (1.0 - omega) * cg%w(fldi)%arr(iarr_all_my(1), i, j, k)/cg%w(fldi)%arr(iarr_all_dn(1), i, j, k) + omega * vi2
            v3 = (1.0 - omega) * cg%w(fldi)%arr(iarr_all_mz(1), i, j, k)/cg%w(fldi)%arr(iarr_all_dn(1), i, j, k) + omega * vi3
            vtot1 = v1
            vtot2 = v2
            vtot3 = v3
            gpcx = cg%w(gpci)%arr(iarr_all_gpcx(ns),i,j,k)
...
#ifndef ISO
               if ( (use_escr_floor)  .and. (new_ec .notequals. escr_floor)) dE = dE + (new_ec - ec)
               if (.not. use_escr_floor) dE = dE + (new_ec - ec)
#endif /* !ISO */
               dFx = dFx + (newf1 - f1)/vmax
               dFy = dFy + (newf2 - f2)/vmax
