   ...
   subroutine apply_scr_source(cg,istep)
      ...
      do ns = 1, scrind%stcosm
         ...
         & i = cg%lhn(xdim,LO):cg%lhn(xdim,HI))
            ...
            coef_11 = 1.0 - rk_coef(istep) * dt * sigmax * vtot1 * v1 * (1.0/vmax) * 4.0/3.0 &
            ...
            coef_22 = 1.0 + rk_coef(istep) * dt * vmax *  sigmax

            coef_31 = -rk_coef(istep) * dt * v2 * sigmay * 4.0/3.0
            coef_33 = 1.0 + rk_coef(istep) * dt * vmax *  sigmay

            coef_41 = -rk_coef(istep) * dt * v3 * sigmaz * 4.0/3.0
            coef_44 = 1.0 + rk_coef(istep) * dt * vmax *  sigmaz
