...
module hydrostatic
! pulled by GRAV
   use constants, only: dsetnamelen, big_float
   ...
   real, allocatable, dimension(:) :: gprofs_ext!< array of gravitational acceleration in a column of subgrid due to external potential
   ...
   subroutine hydrostatic_zeq_densmid(iia, jja, d0, csim2, sd, omega, maxiter, err)
      ...
      logical                     :: self_g = .false.
		...
		if (present(omega)) then
         ...
         om = 0.7
         if (.not. self_g) om = 1.0   ! single pass for no self-gravity case so no relaxation needed
      endif
