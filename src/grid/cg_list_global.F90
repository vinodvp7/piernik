...
!> \brief Register all crucial fields, which we cannot live without
   ...
   subroutine register_fluids(this)
      ...
      use constants,  only: wa_n, fluid_n, uh_n, AT_NO_B, PIERNIK_INIT_FLUIDS, UNSPLIT, xflx_n, yflx_n, zflx_n
...
#endif /* ISO */
#ifdef MAGNETIC
      use constants,  only: mag_n, magh_n, ndims, AT_OUT_B, VAR_XFACE, VAR_YFACE, VAR_ZFACE, &
                            VAR_CENTER, psi_n, psih_n, xbflx_n, ybflx_n, zbflx_n, psiflx_n
      ...
      if (which_solver_type == UNSPLIT) then     !! Only resgister the flux array if we are using unsplit solver
