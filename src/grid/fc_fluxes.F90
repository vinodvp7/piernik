...
!<
   ...
   subroutine initiate_flx_recv(req, cdim, max_level,unsplit)

      use cg_cost_data, only: I_MHD  ! ToDo: for explicit diffusion use I_DIFFUSE or I_REFINE
      use cg_leaves,    only: leaves
      use cg_list,      only: cg_list_element
      use constants,    only: LO, HI, base_level_id,xdim
      ...
      logical, optional,         intent(in)    :: unsplit   ! If true only call req%init once when cdim=xdim
