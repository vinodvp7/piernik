...
!<
   ...
   subroutine update_chspeed()
      ...
      use constants,    only: GEO_XYZ, pMAX, small, DIVB_HDC, RIEMANN
      use dataio_pub,   only: die
      use domain,       only: dom
      ...
      if (which_solver /= RIEMANN) call die("[hdc:update_chspeed] Only Riemann solver has DIVB_HDC implemented")
...
!<
  subroutine glmdamping(half)
      ...
      use cg_leaves,        only: leaves
      use cg_list,          only: cg_list_element
      use constants,        only: psi_n, DIVB_HDC, pMIN, RIEMANN, UNSPLIT
      use dataio_pub,       only: die
      use domain,           only: dom
...
!--------------------------------------------------------------------------------------------
   ...
   subroutine eglm
      ...
      use cg_leaves,  only: leaves
      use cg_list,    only: cg_list_element
      use constants,  only: xdim, zdim, psi_n, GEO_XYZ, half, RIEMANN
      use dataio_pub, only: die
      ...
      if (which_solver /= RIEMANN) call die("[hdc:eglm] Only Riemann solver has DIVB_HDC implemented")
