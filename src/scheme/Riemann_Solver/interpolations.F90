...
subroutine interpol_scr_edge(u, ql, qr)
   ...
   integer :: p
   real :: q(size(u,1), size(u,2))

   ! pack; scale F by 1/vm (your current scaling)
   do p = 1, scrind%stcosm
      q(:, I_ONE   + (p-1)*I_FOUR) =  u(:, I_ONE   + (p-1)*I_FOUR)      ! Ec
      q(:, I_TWO   + (p-1)*I_FOUR) =  u(:, I_TWO   + (p-1)*I_FOUR)/vm   ! F∥
      q(:, I_THREE + (p-1)*I_FOUR) =  u(:, I_THREE + (p-1)*I_FOUR)/vm   ! Ft1
      q(:, I_FOUR  + (p-1)*I_FOUR) =  u(:, I_FOUR  + (p-1)*I_FOUR)/vm   ! Ft2
   end do
   q(:, size(u,2)) = u(:, size(u,2))   ! last column: fluid velocity

   call interp(q, ql, qr, flimiter)
   call cr_fix_edges_from_q(q, ql, qr, scrind%stcosm, dom%nb)
...
contains
   ...
   subroutine cr_fix_edges_from_q(q, ql, qr, nscr, nb)
      ...
      integer :: ncell, nint, i0, i1, s, iE, iN, iv
      real    :: a, b, sE, sN, sV
      ...
      iv = size(q,2)   ! last column is velocity

      ! ---- Left physical interior interface (between i0 and i0+1)
      do s = 1, nscr
         iE = I_ONE + (s-1)*I_FOUR
         iN = I_TWO + (s-1)*I_FOUR
         a  = q(i0+1,iE) - q(i0  ,iE) ;  b = q(i0+2,iE) - q(i0+1,iE) ;  sE = mc(a,b)
         a  = q(i0+1,iN) - q(i0  ,iN) ;  b = q(i0+2,iN) - q(i0+1,iN) ;  sN = mc(a,b)
         ql(i0, iE) = q(i0, iE) + 0.5*sE
         ql(i0, iN) = q(i0, iN) + 0.5*sN
      end do
      a  = q(i0+1,iv) - q(i0,iv) ;  b = q(i0+2,iv) - q(i0+1,iv) ;  sV = mc(a,b)
      ql(i0, iv) = q(i0, iv) + 0.5*sV

      ! ---- Right physical interior interface (between i1-1 and i1)
      do s = 1, nscr
         iE = I_ONE + (s-1)*I_FOUR
         iN = I_TWO + (s-1)*I_FOUR
         a  = q(i1  ,iE) - q(i1-1,iE) ;  b = q(i1-1,iE) - q(i1-2,iE) ;  sE = mc(a,b)
         a  = q(i1  ,iN) - q(i1-1,iN) ;  b = q(i1-1,iN) - q(i1-2,iN) ;  sN = mc(a,b)
         qr(i1-1, iE) = q(i1, iE) - 0.5*sE
         qr(i1-1, iN) = q(i1, iN) - 0.5*sN
      end do
      a  = q(i1,iv) - q(i1-1,iv) ;  b = q(i1-1,iv) - q(i1-2,iv) ;  sV = mc(a,b)
      qr(i1-1, iv) = q(i1, iv) - 0.5*sV
