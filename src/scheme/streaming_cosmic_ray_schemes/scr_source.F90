...
contains
   ...
   subroutine apply_scr_source(cg,istep)
      ...
      &                           first_stage, xdim, ydim, zdim, ndims, I_THREE,rk_coef, gpcr, uh_n, magh_n,&
      &                            sgmn
      ...
      &                           iarr_all_mz, iarr_all_en
      use initscr,  only: vm, disable_streaming, disable_en_source, disable_feedback
      ...
      call update_sigma(cg,istep, .true.)
         ...
         & i = cg%lhn(xdim,LO):cg%lhn(xdim,HI))
            ...
            vtot3 = v3
            sgn_bgpc = 0.0 
            if (sum( cg%w(magi)%arr(xdim:zdim,i,j,k) * cg%w(gpci)%arr(xdim:zdim,i,j,k) , dim=1 ) > 1e-10 )   sgn_bgpc = 1.0     ! Careful with the magic number
            if (sum( cg%w(magi)%arr(xdim:zdim,i,j,k) * cg%w(gpci)%arr(xdim:zdim,i,j,k) , dim=1 ) < -1e-10 )  sgn_bgpc = -1.0    ! Careful with the magic number
            ...
            gpcx = cg%w(gpci)%arr(xdim,i,j,k)
            gpcy = cg%w(gpci)%arr(ydim,i,j,k)
            gpcz = cg%w(gpci)%arr(zdim,i,j,k)

            ec = cg%w(fldi)%arr(1 + ecid,i,j,k)
            f1 = cg%w(fldi)%arr(2 + ecid,i,j,k)
            f2 = cg%w(fldi)%arr(3 + ecid,i,j,k)
            f3 = cg%w(fldi)%arr(ecid +4,i,j,k)
            ...
            sigmax = cg%w(sgmd)%arr(xdim,i,j,k)
            sigmay = cg%w(sgmd)%arr(ydim,i,j,k)
            sigmaz = cg%w(sgmd)%arr(zdim,i,j,k)
            ...
            coef_11 = 1.0 - rk_coef(istep) * dt * sigmax * vtot1 * v1 *(1./vm) * 4.0/3.0 &
            &         - rk_coef(istep) * dt * sigmay * vtot2 * v2 *(1./vm) * 4.0/3.0 &
            &         - rk_coef(istep) * dt * sigmaz * vtot3 * v3 *(1./vm) * 4.0/3.0
            ...
            if (.not. disable_en_source) cg%w(fldi)%arr(1+ecid,i,j,k) = new_ec
            cg%w(fldi)%arr(2+ecid,i,j,k) = newf1
            cg%w(fldi)%arr(3+ecid,i,j,k) = newf2
            cg%w(fldi)%arr(4+ecid,i,j,k) = newf3
         end do
   ...
   subroutine update_gradpc_here(cg, istep)
      ...
      use named_array_list, only: wna, qna
      use constants,        only: pdims, ORTHO1, ORTHO2, I_ONE, LO, HI, &
      &                           first_stage, xdim, ydim, zdim, ndims, I_THREE,rk_coef, gpcr, uh_n, magh_n, last_stage
      use global,           only: integration_order, dt
      use domain,           only: dom
      use fluidindex,       only: iarr_all_swp, flind
      use fluxtypes,        only: ext_fluxes
      ...
      integer                     :: afdim, uhi, magi, gpci, i, ns,d, ecid
      real, pointer               :: T(:,:,:,:)
      type(fxptr)                 :: F(ndims)
      integer, dimension(3,1) :: iarr_all_gpc,iarr_all_fscr

      ecid  = flind%all_fluids(flind%fluids)%fl%end + 1

      iarr_all_gpc(xdim,:) = [xdim]
      iarr_all_gpc(ydim,:) = [ydim]
      iarr_all_gpc(zdim,:) = [zdim]
      iarr_all_fscr(xdim,:) = [ecid + 2]
      iarr_all_fscr(ydim,:) = [ecid + 3]
      iarr_all_fscr(zdim,:) = [ecid + 4]
      T=> null()
      magi   = wna%bi
      if (istep == first_stage(integration_order) .or. integration_order < 2 )  then
         magi   = wna%ind(magh_n)
      ...
      do d = xdim, zdim                          ! component of grad Pc (x,y,z)
         do afdim = xdim, zdim                    ! sweep direction
            ...
            T(iarr_all_gpc(d,1), L(xdim):U(xdim), L(ydim):U(ydim), L(zdim):U(zdim)) = &
            T(iarr_all_gpc(d,1), L(xdim):U(xdim), L(ydim):U(ydim), L(zdim):U(zdim)) + &
            ( F(afdim)%flx(iarr_all_fscr(d,1), L(xdim):U(xdim), L(ydim):U(ydim), L(zdim):U(zdim)) - &
               F(afdim)%flx(iarr_all_fscr(d,1), L(xdim)+shift(xdim):U(xdim)+shift(xdim), &
                                                L(ydim)+shift(ydim):U(ydim)+shift(ydim), &
                                                L(zdim)+shift(zdim):U(zdim)+shift(zdim)) ) / cg%dl(afdim)
         end do
      end do

   cg%w(wna%ind(gpcr))%arr(:,:,:,:) = cg%w(wna%ind(gpcr))%arr(:,:,:,:) / vm
